...
begin
  ...
  begin
    prop.Add('ChanName'+i.ToString, chan._Name);
...
{ Result := PathRelativeToDMXLibrary(FullFilename)+FIXTURESEPARATOR+
 ...
 for chan in FChannels do
   Result := Result+FIXTURESEPARATOR+chan._Name+
...
begin
  ...
  begin
    chan := TDMXChannel.Create;
    chan._Name := libfix.Channels[i].Name;
    chan.ChannelType := libfix.Channels[i].ChannelType;
    chan.Ranges := libfix.Channels[i].Ranges;
...
begin
  ...
  try
    ...
    if not prop.StringValueOf('Name', _Name, SUniverse+aIndex.ToString) then
...
begin
  ...
  try
    ...
    begin
      ...
      for i:=0 to Count-1 do
      begin
        Universes[i]._Name := temp.Universes[i]._Name; // keep the new universe name
        for j:=0 to curUni.FixturesCount-1 do

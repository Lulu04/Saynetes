...
begin
  ...
  begin
    Label19.Caption := FrameViewChannelsList1.Selected[0].Fixture._Name;
    Label20.Caption := FrameViewChannelsList1.Selected[0].Fixture.Description;
    Label21.Caption := FrameViewChannelsList1.Selected[0]._Name;
...
begin
  ...
  if Length(FTargetChannels) > 1 then
    ...
    FShortReadable := FShortReadable + chan._Name + ' '+STo+' '+FormatFloat('0.00',chan.PercentValue*100);
  end;
...
begin
  ...
  if Length(FTargetChannels) > 1 then
    ...
    if chan.Fixture.Description <> '' then FShortReadable := FShortReadable + chan.Fixture.Description+' ';
    FShortReadable := FShortReadable + chan._Name;
  end;
  FDuration := 0.0;
end;
...
begin
  ...
  FShortReadable := FShortReadable+FSourceChannelForCopy._Name+' '+SOn_+' ';
  if Length(FTargetChannels) > 2 then
    ...
    FShortReadable := FShortReadable + targetChan._Name;
  end;
  FDuration := 0.0;
end;

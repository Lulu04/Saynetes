...
TDriverEnttecOpenDmx = class
  ...
  public
    FThread: TOpenDmxThread ; // thread pour l'envoi des trames à répétitions
    constructor Create;
...
begin
  ...
  FillChar(DMXDATA[0], Length(DMXDATA), 0); // start code and data to 0
end;
...
begin
  ...
  while not Terminated do begin
   ...
   FT_W32_WriteFile(ftHandle, @DMXDATA[0], 513, @bytesWritten, nil);
...
begin
  ...
  if FftHandle = INVALID_HANDLE_VALUE then begin
    showmessage('Enttec OPEN DMX - Ouverture de l''interface' + lineending + 'Erreur lors de ''FT_W32_CreateFile''...' );
    Result := FALSE;
    exit;
  end;
  if not FT_W32_GetCommState(FftHandle, @ft_DCB) then begin
  ...
  if not FT_W32_SetCommState(FftHandle, @ft_DCB) then begin
  ...
  FThread := TOpenDmxThread.Create(TRUE);
  FThread.ftHandle := FftHandle;
  FThread.SetRefreshRate(FFrequenceDmx);
  FThread.Start;
...
begin
  if FThread<>NIL then begin
    FThread.Terminate;
    FThread.WaitFor;
    while not FThread.FlagFinish do;
    FThread.Free;
    FThread := NIL;
  end;
...
begin
  FThread.DMXData[aAdresseDmx] := aValeur;
end;

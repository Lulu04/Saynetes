...
begin
  ...
  System.Move(FDataIn[0], data^, expected_length);
...
begin
  ...
  if (res <> FT_OK) or (c <> Length(buf)) then begin
    Log.Warning(Name+' don''t accept to request serial number');
  end else begin
    SetLength(buf, 9);
    c := 0;
    ...
    if (res = FT_OK) and (c = Length(buf))
      ...
      else Log.Warning(Name+' don''t give response to request to serial number');
  end;
end;
...
begin
  ...
  p.CommandLine := 'rmmod ftdi_sio usbserial';   // remove module from kernel
